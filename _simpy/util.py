...
def start_delayed(env, generator, delay):
    ...
    :meth:`~_simpy.core.Environment.process()` starts a process at the current
    simulation time. This helper allows you to start a process after a delay of
    *delay* simulation time units::

        >>> from _simpy import Environment
        >>> from _simpy.util import start_delayed
...
def subscribe_at(event):
    ...
    a :class:`~_simpy.events.Process` to get notified when it terminates.
