"""
This module contains the basic event types used in _simpy.
...
.. autosummary::
    ...
    ~_simpy.events.Event
    ~_simpy.events.Timeout
    ~_simpy.events.Process
    ~_simpy.events.AnyOf
    ~_simpy.events.AllOf
"""
from _simpy._compat import PY2
from _simpy.exceptions import Interrupt, StopProcess
...
class Event(object):
    ...
    def __init__(self, env, agent=None):
        self.env = env
        self.agent = agent
        """The :class:`~_simpy.core.Environment` the event lives in."""
        self.callbacks = []
    ...
    def defused(self):
        ...
        :class:`~_simpy.core.Environment` processes the event (i.e. in
        :meth:`~_simpy.core.Environment.step()`).
        ...
        processed by the :class:`~_simpy.core.Environment`.
    ...
    def __and__(self, other):
        """Return a :class:`~_simpy.events.Condition` that will be triggered if
...
class Timeout(Event):
    """A :class:`~_simpy.events.Event` that gets triggered after a *delay* has
    passed.
...
class Interruption(Event):
    """Immediately schedules an :class:`~_simpy.exceptions.Interrupt` exception
...
class Process(Event):
    ...
    .. note::
       ...
       generators. You can use :meth:~_simpy.core.Environment.exit() as
       a workaround.
...
class ConditionValue(object):
    """Result of a :class:`~_simpy.events.Condition`. It supports convenient
...
class AllOf(Condition):
    """A :class:`~_simpy.events.Condition` event that is triggered if all of
...
class AnyOf(Condition):
    """A :class:`~_simpy.events.Condition` event that is triggered if any of
