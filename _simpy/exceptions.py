"""
_simpy specific exeptions.

"""


class _simpyException(Exception):
    """Base class for all _simpy specific exceptions."""


class Interrupt(_simpyException):
    """Exception thrown into a process if it is interrupted (see
    :func:`~_simpy.events.Process.interrupt()`).
...
class StopProcess(_simpyException):
    """Raised to stop a _simpy process (similar to :exc:`StopIteration`).
