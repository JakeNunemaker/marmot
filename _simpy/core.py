...
from _simpy.exceptions import StopProcess
from _simpy.events import (AllOf, AnyOf, Event, Process, Timeout, URGENT,
                          NORMAL)
...
class BaseEnvironment(object):
    ...
    For example, _simpy defines a :class:`Environment` for simulations with
    a virtual time and and a :class:`~_simpy.rt.RealtimeEnvironment` that
    ...
    def schedule(self, event, priority=NORMAL, delay=0):
        ...
        There are two default priority values, :data:`~_simpy.events.URGENT` and
        :data:`~_simpy.events.NORMAL`.
    ...
    def run(self, until=None):
        ...
        - If it is an :class:`~_simpy.events.Event`, the method will continue
...
class Environment(BaseEnvironment):
    ...
    def peek(self):
        ...
        :data:`~_simpy.core.Infinity` if there is no further event."""
        try:
